...
// Find the broken check_button_cooldown
const ccStart = content.indexOf('def check_button_cooldown(sb):');
if (ccStart < 0) { console.log('not found'); process.exit(1); }

const after = content.substring(ccStart);
const lines = after.split('\n');
let ccEnd = 1;
for (let i = 1; i < lines.length; i++) {
    if (lines[i].match(/^def /) || lines[i].match(/^class /)) { ccEnd = i; break; }
}

const oldCC = lines.slice(0, ccEnd).join('\n');
...
console.log('✅ Fixed check_button_cooldown JS syntax');
